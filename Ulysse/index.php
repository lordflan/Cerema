...
if (isset($_SESSION['login_utilisateur']) && isset($_SESSION['password_utilisateur'])) {
	if ($_SESSION['id_service'] == 1) {				// Initialise, Modifie/Complete, Consulte, Valide		
		include('./accueil_srh.php');
	}
	else if ($_SESSION['id_service'] == 2) {		// Modifie/Complete, Consulte, Valide
		include('./accueil_sii.php');
	}
	else if ($_SESSION['id_service'] == 3) {		// Modifie/Complete, Consulte, Valide
		include('./accueil_spmg.php');
	}
	else if ($_SESSION['id_service'] == 4) {		// Consulte, Valide
		include('./accueil_dir.php');
	}
	else if ($_SESSION['id_service'] == 5) {		// FULL POWER
	...
	<head>
		...
		<script>
			...
			alert('\nMISES A JOUR DE L\'APPLICATION\n\nPlus de détails dans l\'onglet "A PROPOS"');
		</script>
	</head>
	<body>
		<div class='container-fluid m-0 p-0'>
			<nav class='navbar navbar row m-0' style='background-color: #bcd4e6; border-bottom: 1px solid #000; border-top: 1px solid #000'>
				...
				<div class='col'>
					...
					<div class='dropdown-menu'>
						...
						if ((!isset($_SESSION['login_utilisateur'])) || (!isset($_SESSION['password_utilisateur'])))
						{
							echo("<a class='dropdown-item' href='liste.php'>Liste arrivant programmés</a>");
						}
						?>
					</div>
				</div>
			...
			<div id='page'>
				...
				<div class='row justify-content-center m-0'>
					<h4>Accueil utilisateurs (sans login)</h4>
				</div>
				<div class='row justify-content-start m-0 pt-3 pl-3'>
					<div id='faq'>
						...
						<p>En tant que simple utilisateur,<br>
							vous pouvez uniquement consulter la liste<br>
						des arrivants programmés (menu Actions)</p>
						<p>Si vous possédez un login utilisateur,<br>
