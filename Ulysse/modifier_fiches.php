...
$reponse = $bdd->query('SELECT * FROM fiches WHERE etat_fiche = 0');
...
echo("<html lang='fr'>
  ...
  <title>Ulysse | Completer / Modifier fiche</title>
  <!-- FAVICON -->
...
echo("
  ...
  <h4>Compléter / Modifier</h4>
  </div>");
  
while ($donnees = $reponse->fetch())
{
  ?>
  <div class='row justify-content-start m-0 mt-4 pl-3 pb-2' style='background-color: #f2f3f4'>
    <?php
    if (($_SESSION['id_service'] == 1) || ($_SESSION['id_service'] == 2) || ($_SESSION['id_service'] == 5)) {
        echo('<div class="col-4"><span style="letter-spacing: 1px; font-weight: bold">Fiche id '.$donnees['id_fiche'].'</span> - '.$donnees['nom_fiche'].' '.$donnees['prenom_fiche'].'<br>Date limite de la fiche: '.$donnees['limite_fiche'].'</div><div class="col-2"><a href="liste.php?id_fiche='.$donnees['id_fiche'].'" class="btn btn-info mt-2 mr-4">Consulter</a></div><div class="col-4"><a href="modifier.php?id_fiche='.$donnees['id_fiche'].'" class="btn btn-success mt-2">Completer / Modifier</a></div><div class="col-2"><a href="suppression_fiche.php" class="btn btn-danger mt-2 delete">Supprimer</a></div>');
      }
      else {
        echo('<div class="col-4"><span style="letter-spacing: 1px; font-weight: bold">Fiche id '.$donnees['id_fiche'].'</span> - '.$donnees['nom_fiche'].' '.$donnees['prenom_fiche'].'<br>Date limite de la fiche: '.$donnees['limite_fiche'].'</div><div class="col-2"><a href="liste.php?id_fiche='.$donnees['id_fiche'].'" class="btn btn-info mt-2 mr-4">Consulter</a></div><div class="col-4"><a href="modifier.php" class="btn btn-success mt-2">Completer / Modifier</a></div>');
      }
      ?>
  </div>
  <?php
}
echo("<div class='row justify-content-center m-0 pt-4 pb-4'>
  <a href='index.php' class='btn btn-primary btn-lg'>Retour</a>
  </div>
  </div>
  <footer>
  <nav class='navbar navbar row m-0' style='background-color: #bcd4e6; border-bottom: 1px solid #000; border-top: 1px solid #000'>
