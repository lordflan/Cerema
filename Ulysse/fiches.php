...
// On récupère tout le contenu de la table fiches
$resultat = $bdd->query('SELECT * FROM fiches');
...
echo("<html lang='fr'>
 ...
 <title>Ulysse | Consultation fiche</title>
 <!-- FAVICON -->
...
if (isset($_SESSION['login_utilisateur']) && isset($_SESSION['password_utilisateur'])) {
   if ($_SESSION['id_service'] == 1 || $_SESSION['id_service'] == 5) {
      echo("<a class='dropdown-item' href='liste.php'>Liste arrivants programmés</a>
   ...
   }else if ($_SESSION['id_service'] == 2 || $_SESSION['id_service'] == 4) {
      echo("<a class='dropdown-item' href='liste.php'>Liste arrivants programmés</a>
         <a class='dropdown-item' href='fiches.php'>Consulter Fiches</a>
         <a class='dropdown-item' href='modifier_fiches.php'>Completer / Modifier Fiches</a>
         <a class='dropdown-item' href='#'>Clôturer Fiches</a>");
   }else if ($_SESSION['id_service'] == 3) {
      echo("<a class='dropdown-item' href='liste.php'>Liste arrivants programmés</a>
         <a class='dropdown-item' href='fiches.php'>Consulter Fiches</a>
         <a class='dropdown-item' href='modifier_fiches.php'>Completer / Modifier Fiches</a>");
   }
...
if (isset($_SESSION['login_utilisateur']) && isset($_SESSION['password_utilisateur'])) {
   if ($_SESSION['id_service'] == 1 || $_SESSION['id_service'] == 2 || $_SESSION['id_service'] == 3 || $_SESSION['id_service'] == 4) {
      echo("<div class='col'>
         <a class='nav-link dropdown-toggle' data-toggle='dropdown' href='logout.php' role='button' aria-haspopup='true' aria-expanded='false'>");
         echo("<img class='lock' src='./lock.svg' alt='lock'> " . $_SESSION['login_utilisateur'] . " - " . $_SESSION['libellecourt_service'] . '_' . $_SESSION['libelle_role'] . "</a>
         <div class='dropdown-menu ml-5'>
         ...
         echo("<div class='col'>
            <a class='nav-link dropdown-toggle' data-toggle='dropdown' href='logout.php' role='button' aria-haspopup='true' aria-expanded='false'><img class='lock' src='img/lock.svg' alt='lock'>&#9889; " .  $_SESSION['libelle_role'] . " &#9889;</a>
            <div class='dropdown-menu ml-5'>
            <a class='dropdown-item' href='logout.php'>Déconnexion</a>
            </div>
            </div>");
...
while ($donnees = $resultat->fetch())
{
  ...
  <nav class="navbar navbar-light bg-light">
    <span class="navbar-brand mb-0 h1">Fiche id: <?php echo $donnees['id_fiche']; ?></span>
  </nav>
  <div class="row justify-content-start m-0 pl-4 pb-3">
    <?php echo 'Statut fiche: ' .$donnees['statut_fiche']; ?><br>
    <?php echo 'Nom: ' . $donnees['nom_fiche']; ?><br>
    <?php echo 'Prénom: '. $donnees['prenom_fiche']; ?><br>
    <?php echo "Date d'arrivée: " . $donnees['arrivee_fiche']; ?><br>
    <?php echo 'Poste: ' .$donnees['poste_fiche']; ?><br>
    <?php echo 'Date création fiche: ' . $donnees['creation_fiche']; ?><br>
    <?php echo 'Etat fiche: ' . $donnees['etat_fiche']; ?><br>
    <?php echo 'Bâtiment: ' . $donnees['batiment_fiche']; ?><br>
    <?php echo 'Unité: ' . $donnees['unite_fiche']; ?><br>
    <?php echo 'Visa: ' . $donnees['visa_fiche']; ?><br>
    <?php echo 'Assistante: ' . $donnees['assistante_fiche']; ?><br>
    <?php echo 'Téléphone fixe: ' . $donnees['telephonefixe_fiche']; ?><br>
    <?php echo 'Date limite fiche: ' . $donnees['limite_fiche']; ?><br>
    <?php echo 'Service: ' . $donnees['id_service']; ?>
    </div>
    <?php
  }
  echo("<div class='row justify-content-center m-0 mb-4'>
